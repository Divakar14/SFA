...
mongoDB=pymongo.MongoClient('mongodb+srv://demo:aaaaaaaa@cluster0.jlvh1nk.mongodb.net/?retryWrites=true&w=majority')
db=mongoDB['SalesPrediction']
account=db.account

app=Flask(__name__)
app.secret_key = b'\xa5\xed\x83O2B>\x05\x0bdGQ\xc8J\x82\xe7\x9a\x95\xb1|\xc5\x7f\xec\x1c'
CORS(app)
...
def postPrediction(email):
    ...
    if request.method=="POST":
        ...
        mod = sm.tsa.statespace.SARIMAX(new_data, order=(1, 1, 1), seasonal_order=(1, 1, 1, 12), enforce_invertibility=False)
        results = mod.fit()
